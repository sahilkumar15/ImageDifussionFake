# code/DiffusionFake/datasets/base_dataset.py
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        if self.control == True:
            ...
            if self.transform is not None:
                hint = self.transform(image=image)["image"]  # torch CHW if ToTensorV2 used
            else:
                hint = torch.from_numpy(image).permute(2, 0, 1).float()

            hint_ori = torch.from_numpy(image).permute(2, 0, 1).float() * 2.0 - 1.0
