...
class DiffSwap(data.Dataset):
    """
    Image-only dataset. CSV has:
      path,label,source (and optional rel_path, id)
    Returns dict compatible with your eval pipeline:
    ...
    def __init__(
        ...
        if self.split_csv is None:
            raise ValueError("[DiffSwap] split_csv is required.")

        self.df = self._load_csv(self.split_csv)
        self.items = self._build_items(self.df)

        self.real_num = sum(1 for _, y in self.items if int(y) == 0)
        self.fake_num = len(self.items) - self.real_num

        print(
            f"[DiffSwap:{self.split}] images={len(self.items)} "
    ...
    def __getitem__(self, index):
        ...
        if img is None:
            ...
            raise FileNotFoundError(f"[DiffSwap] cannot read: {img_path}")
        ...
        if self.transform is not None:
            ...
            if not isinstance(hint, torch.Tensor):
                raise TypeError(f"[DiffSwap] transform must return torch.Tensor; got {type(hint)}")
        else:
            hint = hint_ori.clone()
    ...
    def _load_csv(self, csv_path: str) -> pd.DataFrame:
        ...
        if not os.path.exists(csv_path):
            raise FileNotFoundError(f"[DiffSwap] split_csv not found: {csv_path}")

        df = pd.read_csv(csv_path)
        for col in ["path", "label"]:
            if col not in df.columns:
                raise ValueError(f"[DiffSwap] CSV must contain '{col}'. got={list(df.columns)}")
        return df
    ...
    def _build_items(self, df: pd.DataFrame):
        ...
        for _, row in df.iterrows():
            ...
            # if user stored relative paths, support it too
            ...
            if not os.path.exists(p):
                missing += 1
                if self.strict_csv:
                    raise FileNotFoundError(f"[DiffSwap] missing image: {p}")
                continue
        ...
        print(f"[DiffSwap:{self.split}] missing_images={missing}")
        return items