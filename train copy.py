...
def main():
    ...
    if _is_rank0(args):
        if getattr(args.wandb, "name", None) is None:
            args.wandb.name = os.path.basename(args.config).replace(".yaml", "")
    ...
    # Let Lightning manage DDP sampler; shuffle=True is okay (it will swap sampler under DDP).
    train_dataloader = DataLoader(
        train_ds,
        batch_size=base_train_dl.batch_size,
        shuffle=True,
        num_workers=base_train_dl.num_workers,
        pin_memory=True,
        drop_last=getattr(base_train_dl, "drop_last", True),
        collate_fn=safe_collate,
        persistent_workers=False if base_train_dl.num_workers == 0 else True,
    )

    val_dataloader = DataLoader(
        val_ds,
        batch_size=base_val_dl.batch_size,
        shuffle=False,
        num_workers=base_val_dl.num_workers,
        pin_memory=True,
        drop_last=getattr(base_val_dl, "drop_last", False),
        collate_fn=safe_collate,
        persistent_workers=False if base_val_dl.num_workers == 0 else True,
    )

    # quick sanity-check batch (rank0 only)
    if _is_rank0(args):
    ...
    trainer = pl.Trainer(
        ...
        accumulate_grad_batches=1,          # set >1 if you want larger effective batch
